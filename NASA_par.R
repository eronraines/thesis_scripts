...
dir<-"/Volumes/WD_BLACK/PAR_NASA"
#get a list of all files with LAI in the name in your directory
files<-list.files(path=dir, pattern='.nc4', full.names = TRUE)
#for calls
setwd(dir)
...
for (file in files){ #this loop translates netcdf stored info into rasters
  ...
  if(k==1){
    temp <- brick(file,varname="par")
  }
  if(k!=1){
    temp2 <-brick(file,varname="par")
...
# adjusting the raster's extent to mor closely match MAP
